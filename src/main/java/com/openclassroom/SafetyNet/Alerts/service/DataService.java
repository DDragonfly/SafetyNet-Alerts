...
/**
 * Service de lecture/écriture des données applicatives depuis/vers un fichier JSON.
 * <p>
 * Charge un {@link com.openclassroom.SafetyNet.Alerts.model.DataWrapper} au démarrage
 * et expose des méthodes CRUD (personnes, casernes, dossiers médicaux).
 * Le chemin et le mode lecture seule sont configurables via propriétés:
 * <ul>
 *   <li><code>safetynet.data.path</code> (ex: <code>classpath:data.json</code> ou chemin fichier)</li>
 *   <li><code>safetynet.data.readOnly</code> (true = pas d'écriture)</li>
 * </ul>
 */
@Service
@Slf4j
public class DataService {
    ...
    /**
     * Initialise le service en chargeant les données depuis {@code dataPath}.
     *
     * @throws IllegalStateException si le fichier n'existe pas ou n'est pas lisible
     */
    @PostConstruct
    public void init() {
    ...
    /**
     * Persiste les données en mémoire vers le fichier JSON défini.
     * Si le mode lecture seule est activé, aucune écriture n’est effectuée.
     */
    public void persist() {
        if (readOnly) {
